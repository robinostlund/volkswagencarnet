...
from pyweconnect.utilities import find_path, is_valid_path
...
class Vehicle:
                    ...
                    if service.get('serviceId', 'Invalid') in self._services.keys():
                        ...
                        if service.get('serviceStatus', {}).get('status', 'Disabled') == 'Enabled':
                            ...
                            if service.get('cumulatedLicense', {}).get('expirationDate', False):
                                data['expiration'] = service.get('cumulatedLicense', {}).get('expirationDate',
    ...
    async def set_climatisation(self, mode='off', spin=False):
        ...
        if self.is_electric_climatisation_supported:
            if mode in ['electric', 'auxiliary']:
                ...
                data = {'action': {
                    'settings': {'climatisationWithoutHVpower': without_hv_power, 'targetTemperature': target_temp,
                                 'heaterSource': mode}, 'type': 'startClimatisation'}}
            elif mode == 'off':
    ...
    def dashboard(self, **config):
        ...
        from pyweconnect.dashboard import Dashboard
    ...
    def last_connected(self):
        ...
        last_connected_utc = self.attrs.get('StoredVehicleDataResponse').get('vehicleData').get('data')[0].get('field')[
            0].get('tsCarSentUtc')
    ...
    def is_last_connected_supported(self):
        """Return when vehicle was last connected to connect servers."""
        if next(iter(next(iter(self.attrs.get('StoredVehicleDataResponse', {}).get('vehicleData', {}).get('data', {})),
                          None).get('field', {})), None).get('tsCarSentUtc', []):
            return True
    ...
    def charging(self):
        ...
        cstate = self.attrs.get('charger', {}).get('status', {}).get('chargingStatusData', {}).get('chargingState',
                                                                                                   {}).get('content',
                                                                                                           '')
    ...
    def charging_time_left(self):
        ...
        if self.external_power:
            minutes = self.attrs.get('charger', {}).get('status', {}).get('batteryStatusData', {}).get(
    ...
    def external_power(self):
        ...
        check = self.attrs.get('charger', {}).get('status', {}).get('chargingStatusData', {}).get(
            'externalPowerSupplyState', {}).get('content', '')
    ...
    def is_external_power_supported(self):
        ...
        if self.attrs.get('charger', {}).get('status', {}).get('chargingStatusData', {}).get('externalPowerSupplyState',
                                                                                             False):
            return True
    ...
    def energy_flow(self):
        ...
        check = self.attrs.get('charger', {}).get('status', {}).get('chargingStatusData', {}).get('energyFlow', {}).get(
            'content', 'off')
        if check == 'on':
            return True
        else:
    ...
    def electric_climatisation(self):
        ...
        climatisation_type = self.attrs.get('climater', {}).get('settings', {}).get('heaterSource', {}).get('content',
                                                                                                            '')
        status = self.attrs.get('climater', {}).get('status', {}).get('climatisationStatusData', {}).get(
            'climatisationState', {}).get('content', '')
    ...
    def is_climatisation_supported(self):
        ...
        response = self.attrs.get('climater', {}).get('status', {}).get('climatisationStatusData', {}).get(
            'climatisationState', {}).get('content', '')
        if response != '':
            return True
    ...
    def window_heater(self):
        ...
        ret = False
        status_front = self.attrs.get('climater', {}).get('status', {}).get('windowHeatingStatusData', {}).get(
            'windowHeatingStateFront', {}).get('content', '')
        if status_front == 'on':
            ret = True

        status_rear = self.attrs.get('climater', {}).get('status', {}).get('windowHeatingStatusData', {}).get(
            'windowHeatingStateRear', {}).get('content', '')
        if status_rear == 'on':
            ret = True
        return ret
    ...
    def is_window_heater_supported(self):
        ...
        if self.is_electric_climatisation_supported:
            if self.attrs.get('climater', {}).get('status', {}).get('windowHeatingStatusData', {}).get(
                    'windowHeatingStateFront', {}).get('content', '') in ['on', 'off']:
                return True
            if self.attrs.get('climater', {}).get('status', {}).get('windowHeatingStatusData', {}).get(
                    'windowHeatingStateRear', {}).get('content', '') in ['on', 'off']:
                return True
    ...
    def pheater_ventilation(self):
        ...
        return self.attrs.get('heating', {}).get('climatisationStateReport', {}).get('climatisationState',
                                                                                     False) == 'ventilation'
